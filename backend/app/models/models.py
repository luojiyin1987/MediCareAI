...
class User(Base):
    ...
    password_hash = Column(
        String(255), nullable=False, comment="Bcrypt hashed password / Bcrypt 哈希密码"
    )
    ...
    full_name = Column(String(255), nullable=False, comment="Full name / 全名")
    phone = Column(String(20), nullable=True, comment="Contact phone / 联系电话")

    # Status Fields | 状态字段
    is_active = Column(
        Boolean, default=True, comment="Account active status / 账户激活状态"
    )
    is_verified = Column(
    ...
    gender = Column(String(10), nullable=True, comment="[Patient] Gender / [患者] 性别")
    address = Column(
        Text,
        nullable=True,
    ...
    # =============================================================================
    title = Column(
        String(50), nullable=True, comment="[Doctor] Professional title / [医生] 职称"
    )
    # e.g., 主任医师, 副主任医师, 主治医师, 住院医师

    department = Column(
        String(100), nullable=True, comment="[Doctor] Department / [医生] 科室"
    )
    ...
    is_verified_doctor = Column(
        Boolean, default=False, comment="[Doctor] License verified / [医生] 资质已认证"
    )
    ...
    # =============================================================================
    admin_level = Column(
        Enum("super", "regular", name="admin_level"),
        nullable=True,
    ...
    doctor_relations = relationship(
        "DoctorPatientRelation",
        back_populates="doctor",
        foreign_keys="DoctorPatientRelation.doctor_id",
    )
    patient_relations = relationship(
        "DoctorPatientRelation",
        back_populates="patient",
        foreign_keys="DoctorPatientRelation.patient_id",
    )
    doctor_verifications = relationship(
        "DoctorVerification",
        back_populates="doctor",
        foreign_keys="DoctorVerification.user_id",
    )
    doctor_comments = relationship(
        "DoctorCaseComment",
        back_populates="doctor",
        foreign_keys="DoctorCaseComment.doctor_id",
    )
    patient_replies = relationship(
        "CaseCommentReply",
        back_populates="patient",
        foreign_keys="CaseCommentReply.patient_id",
    )

    # Internal messaging relationships / 站内信关联
    sent_messages = relationship(
        "InternalMessage",
        back_populates="sender",
        foreign_keys="InternalMessage.sender_id",
    )
    received_messages = relationship(
        "InternalMessage",
        back_populates="recipient",
        foreign_keys="InternalMessage.recipient_id",
    )
    ...
    data_sharing_consents = relationship(
        "DataSharingConsent",
        back_populates="patient",
    ...
    def generate_anonymous_profile(self):
        ...
        self.anonymous_profile = {
            ...
            "gender": self.gender,
            "city_tier": city_tier,
            "city_environment": "urban"
            if self.address and "市" in self.address
            else "rural",
        }
...
class Disease(Base):
    ...
    category = Column(
        String(100), index=True, comment="Disease category for grouping / 疾病分类"
    )
...
class ChronicDisease(Base):
    ...
    icd10_name = Column(
        String(255), nullable=False, comment="ICD-10 official name / ICD-10官方名称"
    )

    # Disease Classification | 疾病分类
    disease_type = Column(
        Enum("chronic", "special", "both", name="disease_type"),
        nullable=False,
        comment="Disease type: chronic/special/both / 疾病类型：慢性病/特殊病/两者皆是",
    )

    # Common Names | 常用名称
    common_names = Column(ARRAY(String), comment="Common aliases / 常用别名列表")

    # Category | 分类
    category = Column(String(100), index=True, comment="Disease category / 疾病分类")
    ...
    # Description | 描述
    description = Column(Text, comment="Disease description / 疾病描述")

    # Medical Notes | 医疗注意事项
    medical_notes = Column(
        Text, comment="Important medical notes for AI / AI诊断医疗注意事项"
    )
    ...
    # Relationships
    patient_conditions = relationship(
        "PatientChronicCondition",
...
class PatientChronicCondition(Base):
    ...
    disease_id = Column(
        UUID(as_uuid=True),
        ForeignKey("chronic_diseases.id"),
        nullable=False,
        index=True,
    )

    # Condition Details | 病情详情
    diagnosis_date = Column(Date, nullable=True, comment="Date of diagnosis / 确诊日期")
    severity = Column(
        Enum("mild", "moderate", "severe", name="condition_severity"),
        nullable=True,
        comment="Condition severity / 病情严重程度",
    )
    notes = Column(Text, comment="Patient notes about condition / 患者备注")
    ...
    chronic_disease = relationship(
        "ChronicDisease", back_populates="patient_conditions"
    )

    # Unique constraint - one disease per patient
    __table_args__ = (
        UniqueConstraint("patient_id", "disease_id", name="unique_patient_disease"),
    )
...
class MedicalCase(Base):
    ...
    disease_id = Column(
        UUID(as_uuid=True), ForeignKey("diseases.id"), nullable=True, index=True
    )
    ...
    is_shared = Column(
        Boolean, default=False, comment="Whether case is shared / 是否已分享"
    )
    share_scope = Column(
        Enum("private", "to_doctor", "platform_anonymous", name="share_scope"),
        default="private",
    ...
    patient = relationship(
        "User", back_populates="medical_cases", foreign_keys=[patient_id]
    )
    disease = relationship("Disease", back_populates="medical_cases")
    medical_documents = relationship(
        "MedicalDocument", back_populates="medical_case", cascade="all, delete-orphan"
    )
...
class MedicalDocument(Base):
    ...
    medical_case_id = Column(
        UUID(as_uuid=True), ForeignKey("medical_cases.id"), nullable=False, index=True
    )
    ...
    extracted_content = Column(
        JSONB, comment="Raw MinerU extracted content / MinerU原始提取内容"
    )

    # PII-Cleaned Content (for sharing) | PII清理后内容（用于分享）
    cleaned_content = Column(
        JSONB, comment="PII-cleaned content for sharing / PII清理后用于分享的内容"
    )

    # PII Cleaning Metadata | PII清理元数据
    pii_cleaning_status = Column(
        Enum("pending", "completed", "failed", name="pii_status"),
        default="pending",
        comment="PII cleaning status / PII清理状态",
    )
    pii_detected = Column(
        JSONB, default=list, comment="List of detected PII / 检测到的PII列表"
    )
    cleaning_confidence = Column(
        Float, comment="PII cleaning confidence score / PII清理置信度"
    )
...
class AIFeedback(Base):
    ...
    knowledge_sources = Column(
        JSONB, default=list, comment="Knowledge base chunks used / 使用的知识库块"
    )
...
class UserSession(Base):
    ...
    # Session metadata
    login_role = Column(
        String(20), comment="Role at login (for role switching) / 登录时的角色"
    )
...
class DataSharingConsent(Base):
    ...
    # Share Scope | 分享范围
    target_doctor_id = Column(
        UUID(as_uuid=True),
        ForeignKey("users.id"),
        nullable=True,
        comment="Target doctor if specific / 指定医生",
    )
    disease_category = Column(
        String(100), nullable=True, comment="Limit to disease category / 限制疾病分类"
    )

    # Consent Content | 同意内容
    consent_version = Column(
        String(20), nullable=False, comment="Consent form version / 同意书版本"
    )
    consent_text = Column(
        Text, nullable=False, comment="Full consent text / 完整同意书文本"
    )

    # Signature Info | 签署信息
    ip_address = Column(String(45), comment="IP address when signing / 签署时IP地址")
    user_agent = Column(Text, comment="User agent when signing / 签署时浏览器信息")
    ...
    # Relationships
    patient = relationship(
        "User", back_populates="data_sharing_consents", foreign_keys=[patient_id]
    )
...
class SharedMedicalCase(Base):
    ...
    consent_id = Column(
        UUID(as_uuid=True), ForeignKey("data_sharing_consents.id"), nullable=False
    )

    # Anonymized Patient Profile | 匿名化患者资料
    anonymous_patient_profile = Column(
        JSONB, nullable=False, comment="Anonymized profile / 匿名化资料"
    )
...
class DoctorPatientRelation(Base):
    ...
    status = Column(
        Enum("pending", "active", "terminated", name="relation_status"),
        default="pending",
    ...
    initiated_by = Column(
        Enum("patient_at", "doctor_request", "platform_match", name="initiation_type"),
        nullable=False,
        comment="How relationship started / 关系发起方式",
    )

    # Share Scope | 分享范围
    share_all_cases = Column(
        Boolean, default=False, comment="Share all cases / 分享所有病例"
    )
    shared_case_ids = Column(
        JSONB, default=list, comment="Specific shared case IDs / 指定分享的病例ID"
    )

    # Messages | 消息
    patient_message = Column(Text, comment="Message from patient / 患者留言")
    doctor_response = Column(Text, comment="Doctor response / 医生回复")
    ...
    # Relationships
    patient = relationship(
        "User", back_populates="patient_relations", foreign_keys=[patient_id]
    )
    doctor = relationship(
        "User", back_populates="doctor_relations", foreign_keys=[doctor_id]
    )
...
class DoctorCaseComment(Base):
    ...
    # Foreign Keys
    shared_case_id = Column(
        UUID(as_uuid=True),
        ForeignKey("shared_medical_cases.id"),
        nullable=False,
        index=True,
    )
    doctor_id = Column(
    ...
    content = Column(Text, nullable=False, comment="Comment content / 评论内容")

    # Professional Info | 专业信息
    doctor_specialty = Column(
        String(200), comment="Doctor's specialty at time of comment / 评论时医生专业"
    )
    doctor_hospital = Column(
        String(255), comment="Doctor's hospital at time of comment / 评论时医院"
    )

    # Visibility | 可见性
    is_public = Column(
        Boolean, default=True, comment="Visible to other doctors / 对其他医生可见"
    )

    # Status | 状态
    status = Column(
        Enum("active", "edited", "hidden", name="comment_status"),
        default="active",
    ...
    original_content = Column(
        Text, nullable=True, comment="Original content before edit / 编辑前的原始内容"
    )
    ...
    doctor = relationship(
        "User", back_populates="doctor_comments", foreign_keys=[doctor_id]
    )
    patient_replies = relationship(
        "CaseCommentReply",
        back_populates="doctor_comment",
        cascade="all, delete-orphan",
    )
...
class CaseCommentReply(Base):
    ...
    # Foreign Keys
    doctor_comment_id = Column(
        UUID(as_uuid=True),
        ForeignKey("doctor_case_comments.id"),
        nullable=False,
        index=True,
    )
    patient_id = Column(
    ...
    # Reply Content
    content = Column(
        Text, nullable=False, comment="Patient reply content / 患者回复内容"
    )

    # Status
    status = Column(
        Enum("active", "hidden", name="reply_status"),
        default="active",
...
class VectorEmbeddingConfig(Base):
    ...
    # Basic Info | 基本信息
    name = Column(String(100), nullable=False, comment="Configuration name / 配置名称")
    provider = Column(
        String(50), nullable=False, comment="Provider: openai, local, etc. / 提供商"
    )
    model_id = Column(String(100), nullable=False, comment="Model ID / 模型ID")
    ...
    vector_dimension = Column(
        Integer, default=1536, comment="Embedding dimension / 嵌入维度"
    )
    max_input_length = Column(
        Integer, default=8192, comment="Max input length / 最大输入长度"
    )

    # Status | 状态
    is_active = Column(
        Boolean, default=False, comment="Is active config / 是否为活跃配置"
    )
    is_default = Column(
        Boolean, default=False, comment="Is default config / 是否为默认配置"
    )

    # Test Status | 测试状态
    last_tested_at = Column(DateTime(timezone=True), nullable=True)
    test_status = Column(
        Enum("untested", "success", "failed", name="test_status"),
        default="untested",
        comment="Connection test status / 连接测试状态",
    )
    test_error_message = Column(
        Text, nullable=True, comment="Error message if test failed / 测试失败错误信息"
    )
...
class KnowledgeBaseChunk(Base):
    ...
    disease_category = Column(
        String(100), index=True, comment="Disease category / 疾病分类"
    )
    document_title = Column(String(255))
    section_title = Column(String(255), comment="Section title / 章节标题")

    # Chunk Content | 分块内容
    chunk_index = Column(
        Integer, nullable=False, comment="Chunk sequence number / 分块序号"
    )
    chunk_text = Column(
        Text, nullable=False, comment="Original text content / 原始文本内容"
    )
    chunk_text_hash = Column(
        String(64),
        unique=True,
    ...
    embedding = Column(JSONB, nullable=True, comment="Vector embedding / 向量嵌入")
    embedding_model_id = Column(
        String(100), comment="Model used for embedding / 用于嵌入的模型"
    )

    # Retrieval Statistics | 检索统计
    retrieval_count = Column(
        Integer, default=0, comment="Number of times retrieved / 被检索次数"
    )
    avg_relevance_score = Column(
        Float, nullable=True, comment="Average relevance score / 平均相关度分数"
    )
...
class CaseKnowledgeMatch(Base):
    ...
    # Query Info | 查询信息
    query_text = Column(
        Text, nullable=False, comment="Case summary as query / 病例摘要作为查询"
    )
    query_embedding = Column(JSONB, nullable=True)

    # Matched Results | 匹配结果
    matched_chunks = Column(
        JSONB, nullable=False, comment="Top-K matched chunks / Top-K匹配的知识块"
    )
    # Format: [{"chunk_id": "...", "relevance_score": 0.95, "chunk_text": "..."}]

    # Used Knowledge Sources | 使用的知识源
    knowledge_sources = Column(
        JSONB, default=list, comment="Disease categories used / 使用的疾病分类"
    )

    # AI Selection Reasoning | AI选择说明
    selection_reasoning = Column(
        Text, comment="Why these knowledge bases were selected / 选择理由"
    )
...
class SystemResourceLog(Base):
    ...
    cpu_percent = Column(Float, comment="CPU usage percentage / CPU使用率")
    memory_percent = Column(Float, comment="Memory usage percentage / 内存使用率")
    disk_percent = Column(Float, comment="Disk usage percentage / 磁盘使用率")

    # Container Status | 容器状态
    container_status = Column(
        JSONB, default=dict, comment="Docker container status / Docker容器状态"
    )
    # Format: {"medicare_backend": {"status": "running", "cpu": 10.5, "memory": 256}}

    # Database Metrics | 数据库指标
    db_connections = Column(Integer, comment="Active DB connections / 活跃数据库连接数")
    db_query_time_avg = Column(Float, comment="Average query time / 平均查询时间")

    # Alert Level | 警告级别
    alert_level = Column(
        Enum("info", "warning", "critical", name="alert_level"),
        default="info",
        comment="Alert level / 警告级别",
    )
    alert_message = Column(Text, nullable=True, comment="Alert message / 警告信息")
...
class AIDiagnosisLog(Base):
    ...
    request_duration_ms = Column(
        Integer, comment="Request duration in ms / 请求耗时毫秒"
    )
    ...
    # Result Status | 结果状态
    status = Column(
        Enum("success", "timeout", "error", "rate_limited", name="ai_status"),
        nullable=False,
    )
    ...
    anomaly_reason = Column(Text, nullable=True, comment="Anomaly reason / 异常原因")
...
class AdminOperationLog(Base):
    ...
    operation_details = Column(
        JSONB, default=dict, comment="Operation details / 操作详情"
    )
    ip_address = Column(INET)
    user_agent = Column(Text)
...
class DoctorVerification(Base):
    ...
    status = Column(
        Enum("pending", "approved", "rejected", "revoked", name="verification_status"),
        default="pending",
        nullable=False,
    )
    ...
    # License Document / 执业证书文件
    license_document_path = Column(
        String(500),
        nullable=True,
        comment="Path to uploaded license document / 上传的执业证书文件路径",
    )
    license_document_filename = Column(
        String(255),
        nullable=True,
        comment="Original filename of license document / 执业证书原始文件名",
    )

    # Relationships
    doctor = relationship(
        "User", foreign_keys=[user_id], back_populates="doctor_verifications"
    )
    verifier = relationship("User", foreign_keys=[verified_by])


# =============================================================================
# Internal Message Model | 站内信模型
# =============================================================================
class InternalMessage(Base):
    """
    Internal Message Model / 站内信模型

    Supports doctor-to-admin messaging for bug reports and feedback.
    Doctors can only send messages to admins, not to other doctors.
    支持医生向管理员发送消息（用于bug反馈等），医生之间不能互发消息。
    """
    ...
    # Sender and Recipient / 发送者和接收者
    sender_id = Column(
        UUID(as_uuid=True),
        ForeignKey("users.id", ondelete="SET NULL"),
        nullable=False,
        index=True,
        comment="Message sender / 消息发送者",
    )
    recipient_id = Column(
        UUID(as_uuid=True),
        ForeignKey("users.id", ondelete="SET NULL"),
        nullable=False,
        index=True,
        comment="Message recipient (must be admin) / 消息接收者（必须是管理员）",
    )
    ...
    is_read = Column(
        ...
        default=False,
        nullable=False,
        comment="Whether message has been read / 消息是否已读",
    )
    read_at = Column(
        DateTime(timezone=True),
        nullable=True,
        comment="When message was read / 消息阅读时间",
    )

    # Reply Support / 回复支持
    parent_id = Column(
        UUID(as_uuid=True),
        ForeignKey("internal_messages.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
        comment="Parent message ID for replies / 父消息ID（用于回复）",
    )
    ...
    created_at = Column(
        ...
        server_default=func.now(),
        nullable=False,
        comment="When message was created / 消息创建时间",
    )
    updated_at = Column(
        DateTime(timezone=True),
        server_default=func.now(),
        onupdate=func.now(),
        nullable=False,
        comment="When message was last updated / 消息最后更新时间",
    )
    ...
    # Indexes / 索引
    __table_args__ = (
        Index("ix_internal_messages_sender_created", "sender_id", "created_at"),
        Index("ix_internal_messages_recipient_created", "recipient_id", "created_at"),
        Index("ix_internal_messages_unread", "recipient_id", "is_read"),
    )
